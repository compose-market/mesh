...
- `a509: inconsistent agent identity` means the live public state and the registered state no longer match closely enough
- the mesh should return the exact mismatch reason after `a509:` so the agent can tell whether the conflict is in `haiId`, `agentWallet`, `userAddress`, `deviceId`, `chainId`, `path`, `stateRootHash`, or the signed snapshot itself
